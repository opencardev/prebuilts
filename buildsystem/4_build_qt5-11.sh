...
cd $HOME/qt511
if ! [ -f qt-everywhere-src-5.11.3.tar.xz ]; then
    wget https://download.qt.io/official_releases/qt/5.11/5.11.3/single/qt-everywhere-src-5.11.3.tar.xz
fi

# Unpack source
cd $HOME/qt511/src
if ! [ -d qt-everywhere-src-5.11.3 ]; then
    echo "Unpacking archive..."
    pv -p -w 80 $HOME/qt511/qt-everywhere-src-5.11.3.tar.xz | tar -J -xf - -C $HOME/qt511/src
fi

# Switch to build directory and build
cd $HOME/qt511_build

$HOME/qt511/src/qt-everywhere-src-5.11.3/configure \
